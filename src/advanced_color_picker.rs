    ...
    fn load_colors(only_named: bool) -> Vec<PantoneColor> {
        ...
        for line in include_str!("set1.txt").lines() {
            ...
            if line.starts_with("rgb") {
                ...
                if let (Ok(r),Ok(g),Ok(b)) = (nums[0].parse::<u8>(),nums[1].parse::<u8>(),nums[2].parse::<u8>()) {
                    color = Some([r as f32 / 255.0, g as f32 / 255.0, b as f32 / 255.0, 1.0]);
                }
            }

            let mut all_some = false;
            if let (Some(name), Some(code), Some(category), Some(color)) = (&name, &code, &category, &color) {
...
impl AdvancedColorPicker {
    ...
    pub fn search_text(&mut self) {
        ...
        self.pantone_colors.sort_by_cached_key(|x| ((alg.for_str(&x.code, &self.search_code).ndist() + alg.for_str(&x.name, &self.search_name).ndist()) * 100000.0) as i32);
    ...
    pub fn render(&mut self, ui: &Ui) {
        ...
        if let Some(_tab_bar) = ui.tab_bar("color tab bar") {
            if let Some(tab_iter) = ui.tab_item("Color Picker") {
                ...
                ui.set_next_item_width(0.6*ui.content_region_avail()[0].min(ui.content_region_avail()[1]));
                ...
                .build();
                ui.next_column();
                if ui.button(format!("Switch to {}", match self.color_picker_mode {
            ...
            if let Some(tab_iter) = ui.tab_item("Pantone Colors") {
                ...
                for i in 0..(self.result_count as usize).min(self.pantone_colors.len()) {
                    ...
                    ui.same_line();
                    let b = ui.button(if color.name.is_empty() {format!("{:?}",color.code)} else {color.name.clone()});
