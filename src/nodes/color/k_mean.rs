...
impl MyNode for PalletGenNode {
    ...
    fn edit_menu_render(&mut self, ui: &imgui::Ui, renderer: &mut Renderer, storage: &Storage) {
        ...
        ui.input_float("lightness weight",&mut self.lightness_weight).build();
        ui.input_float("sampling_factor",&mut self.sampling_factor).build();
    ...
    fn run(
        ...
        let data = input_texture.read_to_pixel_buffer().map().iter().map(|x| [x.0,x.1,x.2]).collect::<Vec<[u8;3]>>();

        if let Some(frozen_pallet) = &self.frozen_pallet {
            if frozen_pallet.len() == self.pallet_size as usize * 3 {
                texture2.write(glium::Rect{left:0, bottom:0, width:self.pallet_size as _, height:1}, RawImage2d::<u8>::from_raw_rgb(frozen_pallet.to_vec(), (self.pallet_size as u32,1)));
                return true;
            }
        }
        
        let new_hash = calculate_hash(&(&data, self.pallet_size, (self.lightness_weight.clone() * 1000000.0) as i32, (self.sampling_factor.clone()*1000000.0) as i32));
    
        if !self.map.contains_key(&new_hash) {
        let mut image_data = Vec::new();
        // should probably do something about that .cloned
        if let Ok(mut pallet) = Okolors::try_from(data.iter().cloned().map(|x|
            Srgb::new(x[0],x[1],x[2]).into()
        ).collect::<Vec<Srgb<u8>>>().as_slice()) {
        ...
        if (self.map.get(&new_hash)).unwrap_or(&vec![]).len() == self.pallet_size as usize * 3 {
            ...
            texture2.write(glium::Rect{left:0, bottom:0, width:self.pallet_size as _, height:1}, RawImage2d::<u8>::from_raw_rgb(self.map.get(&new_hash).unwrap().to_vec(), (self.pallet_size as u32,1)));
        }
        return true;
    }
