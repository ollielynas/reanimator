...
impl MyNode for LayerTrailNode {
    ...
    if self.last_frame.is_none() || self.last_frame.as_ref().unwrap().width() != texture.width() || self.last_frame.as_ref().unwrap().height() != texture.height()  {
        let new_texture = match Texture2d::empty(&storage.display, texture.width(), texture.height()) {
            Ok(a) => a,
            Err(_) => {return false}
        };
        ...
        }, &new_texture.as_surface(), &BlitTarget {
            left: 0,
            bottom: 0,
    ...
    let texture2 = storage.get_texture(&output_id).unwrap();
            ...
            texture2.as_surface().blit_color(&Rect {
                left: 0,
                bottom: 0,
                width: texture2.width(),
                height: texture2.height(),
            }, &last_frame.as_surface(), &BlitTarget {
                left: 0,
                bottom: 0,
