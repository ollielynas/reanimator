...
impl Project {
    pub fn render_local_files(
        ...
        ui.window("batch editor")
            ...
            .size(
                [
                    window_size.x - sidebar_params.left_sidebar_width,
                    window_size.y - sidebar_params.menu_bar_size[1],
                ],
                imgui::Condition::Always,
            )
            .no_decoration()
            ...
            .build(|| {
                    ...
                    if let Some(files) = files {
                        for f in files {
                            let a = fs::copy(
                                ...
                                self.storage
                                    .project_root
                                    .join((f.clone()).file_name().unwrap_or(OsStr::new("error"))),
                            );
                            log::info!("{a:?}");
                        }
                    }
                    ...
                    if let Some(folder) = folder {
                        ...
                        if let Ok(files) = files {
                            for f in files {
                                let a = fs::copy(
                                    f.clone(),
                                    self.storage.project_root.join(
                                        (f.clone()).file_name().unwrap_or(OsStr::new("error")),
                                    ),
                                );
                                log::info!("{a:?}");
                            }
                ...
                link_widget(
                    ui,
                    self.storage.project_root.display().to_string(),
                    self.storage.project_root.display().to_string(),
                );
                ...
                if ui.button("Name") && self.project_settings.local_files.files.len() >= 2 {
                    ...
                    let last = self.project_settings.local_files.files
                        [self.project_settings.local_files.files.len() - 1]
                        .path
                        .clone();
                    self.project_settings
                        .local_files
                        .files
                        .sort_by_key(|f| f.name());
                    let first2 = self.project_settings.local_files.files[0].path.clone();
                    let last2 = self.project_settings.local_files.files
                        [self.project_settings.local_files.files.len() - 1]
                        .path
                        .clone();
                ...
                ui.child_window("files").build(|| {
                    ...
                    if remove_item < self.project_settings.local_files.files.len() {
                        let _ = fs::remove_file(
                            &self.project_settings.local_files.files[remove_item].path,
                        );
                        self.project_settings.local_files.reload(&self.storage);
                    }
                });
